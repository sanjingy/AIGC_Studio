...
| 02 | 项目与工作区 | 部分实现（后端已通，freeflow 已独立闭环） | [02_PROJECT_WORKSPACE.md](./modules/02_PROJECT_WORKSPACE.md) |
...
| 08 | 任务、队列与实时进度 | 部分实现（状态机/Outbox/SSE 已通，freeflow 任务页已读 `tasks`；队列不分类） | [08_TASK_REALTIME.md](./modules/08_TASK_REALTIME.md) |
...
| 11 | Web 工作台与交互 | 部分实现（旧壳已删，freeflow 唯一壳，推进/审批/返工/出图闭环；字段级编辑、每镜选模型、逐镜 MP4 未做） | [11_WEB_WORKBENCH.md](./modules/11_WEB_WORKBENCH.md) |
