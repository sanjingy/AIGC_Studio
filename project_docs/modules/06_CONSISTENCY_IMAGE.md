...
- 图片文件与通用元数据归资产模块（07）；执行状态归 `tasks`（ADR-008）；费用归计费（09）；
  模型选择归 Gateway（05）。
- **系统提供三套锁定风格词，Agent 合成最终提示词，校验完整保留**（ADR-036）。
  9 月 13 日整改由 `prompting` 模块承担准备、缓存、版本检查与存档；历史
  `compose.py` 仅保留素材描述、旧兼容工具及历史单测，不能再作为生产出图兜底。

### 2026-09-13 整改契约（实施验收中，未部署）

本节覆盖下文 9 月 6 日基线中「确定性合成」「剥离风格词」「非测试缺 Key 用 Mock」
三项旧行为。详见 ADR-038 与 A/B/C 整改计划。新增 `prompts/{kind}/{subject_key}`
GET/POST、`generation-records` 列表/详情；三类出图接受可选 `prompt_run_id`。
角色立绘、场景四视图、单镜首帧、逐镜视频词共用模型合成与校验路径。非测试环境
缺少可用 Provider 时明确失败；测试占位只在 `ENV=test` 使用。

原版画风目录迁移 `a8c3e91d6402` 增加 `chaoge_v1_01..08`，保留三套描述词全文。
不修改已锁定项目，也不改变旧默认值。目录中的色调留空，全剧色调由 B4 独立推导；
存量数据仍可使用已有色调。缺场景/角色档案返回 409；未知光照引用保留既有默认
光照回退并记录实际选择。完整验收结果另记，不把契约当作已上线证明。
...
- **FR-CONS-012**：**景别 / 角度 / 运镜必须进出图提示词**。今天 `compose_shot` 只读
  `content` / `character_refs` / `scene_ref`（外加 `index` 算 seed），分镜表上的
  景别、角度、运镜三列**从未影响过任何一张图**。
  2026-09-05 之前这三个字段在界面上是只读的，没人能试出来；字段级编辑
  （FR-WEB-004）上线后用户会立刻撞上"把大全景改成特写、重出图一模一样"。
  界面没有撒谎（过期标记说的是"可能不符"），但这是个必然被投诉的洞。
