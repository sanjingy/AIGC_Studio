...
| FR-WEB-004 | 故事 / 角色 / 场景 / 分镜的字段级修改走 ADR-029 的 Patch 端点，刷新后仍在；不再有"只存本地"的可编辑控件 | ADR-029；决策记录 §3.2 |
...
| 字段级变更历史 UI / 批次撤销 UI | P2（后端 ADR-029 已有，排在 FR-WEB-004 之后） | — |
...
`components/freeflow/project/storyboard-editor.tsx`：

| 行 | 缺口 |
|---|---|
| `:184` | "右栏字段改动只存在本地，不写回后端" |
| `:265` | 增删镜头未接（后端目前只能整段重生成 `/revise`，没有单镜增删接口） |
| `:332` | 镜时长是本地草稿。按 §11.1 裁决 3，时长要作为**可选字段加到 `StoryboardShot`**，由确定性代码在 Agent 产出后填写，且时长可被用户 Patch、段数不可 |
| `:414` | 情绪字段后端 schema 里没有，是纯原型演示 |
| `:432` | 镜头级参考图上传未接（后端 `advance` / `revise` 只收文本，既没有字段也没有上传通道） |

ADR-029 的三条 Patch 端点后端已有（`apps/api/modules/content/`，已挂进
`main.py:150`），**前端一行没接**。这是 FR-WEB-004 的全部内容。
...
| 03 内容与故事 | ADR-029 的 Patch / 修订历史 / 批次撤销端点 | 后端已有、前端未接，是 FR-WEB-004 的全部内容 |
