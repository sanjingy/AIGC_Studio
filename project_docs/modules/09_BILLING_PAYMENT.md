...
> 2026-09-14 发布收口契约（待实施）：删除公开 `/credits/topup`，新增
> `/credits/admin-grant`。平台管理员由 `ADMIN_USER_IDS` 用户 UUID 白名单确定，
> 默认为空；组织 owner 不视为平台管理员。请求为 `org_id`、正整数 `amount`
> （最多 1,000,000）和非空 `note`（最多 500 字），必须带 `Idempotency-Key`。
> 发放只写 `ADMIN_GRANT`、`is_bonus=true`，流水保存认证用户的 `operator_id`。
> 同一操作者、目标组织和幂等键重复请求只入账一次，金额/备注不同则 409。
> 未登录 401、非管理员 403、目标组织不存在 404；权限拒绝不得写入账务。
> 该契约落实决策记录 §11.2；以下旧 topup 描述在实施完成后逐项更新。
