...
> 最近核对：2026-09-05
>
> ⚠️ **2026-09-05：§4.4 的 `VideoModelSpec` 已被 ADR-034 推翻，尚未重写。**
> `max_segment_ms: int` 这个标量表达不了真实供应商的能力：时长是**离散档位集合**，
> 还会被分辨率和"带不带参考图"二次收窄。另外该契约缺三样东西：参考图数量上限、
> 参考音频、**模型自带音轨的形态**（缺最后一样会做出双音轨的 MP4）。
