...
> 最近核对：2026-09-05
>
> ⚠️ **2026-09-05：本文档的时长与切段设计已被 ADR-034 推翻，尚未重写。**
> §9.1 的段数算法（`segments = ceil(shot_ms / max_segment_ms)` + 平均分配）会算出
> **在真实供应商上非法的时长值**——可灵只接受 5/10 秒，Hailuo 只接受 6 秒。
> §5 的 `shot_segments` / `segment_videos` 两张表按 ADR-034 第 4 条**暂不建**。
> 动手实现前必须先读 ADR-034 与 ADR-035，并按它们重写本文档。
