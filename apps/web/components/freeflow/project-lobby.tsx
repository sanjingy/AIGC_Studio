...
import { AlertTriangle, ArrowRight, CirclePlus, FolderKanban, Loader2, PlayCircle, ShieldAlert } from "lucide-react";
...
export function ProjectLobby() {
  ...
  return <div className="mx-auto flex w-full max-w-[1560px] flex-col gap-6 p-4 lg:p-6 2xl:p-8">
    ...
    {attention.length > 0 && <section aria-labelledby="attention" className="border-t border-border pt-5"><div className="mb-3 flex items-center gap-2"><ShieldAlert aria-hidden className="size-4 text-running" /><h2 id="attention" className="text-base font-semibold text-fg">需要你处理</h2></div><div className="grid gap-px overflow-hidden border border-border bg-border md:grid-cols-2 xl:grid-cols-3">{attention.map((item) => <Link key={`${item.project.id}-${item.kind}`} href={item.href} className="group flex min-h-20 items-center gap-3 bg-surface p-3 transition-colors duration-150 hover:bg-surface-2 focus-visible:outline-2 focus-visible:outline-primary"><AlertTriangle aria-hidden className={cn("size-4 shrink-0", item.kind === "failed" ? "text-danger" : "text-running")} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-fg">{item.project.title}</span><span className="block text-xs text-fg-subtle">{item.label}</span></span><ArrowRight aria-hidden className="size-4 text-fg-subtle group-hover:text-primary" /></Link>)}</div></section>}
  </div>;
}

function ProjectCard({ project }: { project: Project }) { const active = ["routing", "producing", "review"].includes(project.status); return <Link href={`/freeflow/projects/${project.id}/overview`} className="group flex min-h-40 flex-col border border-border bg-surface p-4 transition-colors duration-150 hover:border-border-strong hover:bg-surface-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"><div className="flex items-start justify-between gap-3"><FolderKanban aria-hidden className="size-5 text-primary" /><span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs", active ? "bg-running-soft text-running" : "bg-surface-2 text-fg-subtle")}><PlayCircle aria-hidden className="size-3" />{STAGE[project.status] ?? project.status}</span></div><h3 className="mt-auto truncate text-base font-semibold text-fg">{project.title}</h3><p className="mt-1 text-xs text-fg-subtle">创建于 {new Date(project.created_at).toLocaleDateString("zh-CN")}</p><div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-xs text-fg-subtle"><span>{project.route_type ?? "等待路线判断"}</span><ArrowRight aria-hidden className="size-3.5 group-hover:text-primary" /></div></Link>; }
...
function EmptyProject({ onCreate }: { onCreate: () => void }) { return <div className="flex min-h-56 flex-col items-center justify-center border border-dashed border-border-strong bg-surface p-6 text-center"><FolderKanban aria-hidden className="size-6 text-fg-subtle" /><h3 className="mt-3 text-base font-semibold text-fg">从一个项目开始</h3><p className="mt-1 max-w-sm text-sm text-fg-subtle">创建项目后，工作台会在这里显示真实的故事、审核和出图进度。</p><button type="button" onClick={onCreate} className="mt-4 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-fg">新建项目</button></div>; }
