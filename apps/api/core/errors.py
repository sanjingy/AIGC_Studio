...
ERRORS: dict[str, ErrorSpec] = {
    ...
    "consistency.profile.missing": ErrorSpec(
        ...
        # 缺了就直说缺哪一步，别让用户拿到一个"生成失败"却不知道该回去做什么。
        #
        # **文案必须泛化到"角色或场景"**：同一个错误码在角色档案缺失和场景档案
        # 缺失两条路径上都会抛（`prompting.context._characters` / `._scenes`），
        # 而它原来只说"还没有角色设定"——缺场景的用户照着做会去重跑角色阶段，
        # 跑完回来仍然出不了图，而错误一个字都没变。具体缺哪一步在 `detail`
        # 的 `missing_stage` 里，界面要指到那一步就读它。
        "consistency.profile.missing",
        409,
        "还没有生成这一步需要的一致性档案（角色或场景），请先完成对应的档案步骤再出图",
    ),
