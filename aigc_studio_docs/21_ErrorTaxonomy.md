...
| **自带 Key 被拒（BYOK）** | 用户自己的 Key 失效/欠费/被封 | ❌ | ❌ 见下 | 释放预扣 | ❌ |
...
| **前置条件缺失** | 还没有角色档案就要出图 | ❌ | ❌ | 尚未预扣 | ❌ |
| **Agent 输出非法** | 结构化输出校验失败 | ✅ 2 次 | 换 LLM | — | ❌ |
| **系统 Bug** | 未捕获异常 | ❌ | ❌ | 全额退 | ❌ |

**BYOK 那一行为什么不换 Provider**（ADR-027）：用户只为这个能力配了一家的
Key，"轮换到另一家"这个选项对他并不存在；而换回**平台**的 Key 更不行——
计费此刻已经按 BYOK 折扣算过了（ADR-025），平台 Key 顶上去就是平台掏钱、
用户按折扣价付款。同一把 Key 换个模型仍然允许（同账号同价），
但账号级的失败（鉴权、欠费）连模型都不换：换个模型问第二遍不会得到
别的答案，只是白打一次上游。

**「前置条件缺失」这一行不是补白**（`consistency.profile.missing`）。出图要拿角色资产包和风格档案合成提示词，两者在角色档案那一步才落库；缺了就在建任务之前拦下来，一分钱都不预扣。它必须有自己的码而不是笼统的 422——用户要知道的是「回去把角色那一步跑完」，不是「参数有误」。

对应的错误码是 `provider.byok.rejected`。它与
`provider.account.insufficient` 的区别只有一个，但这个区别决定了**谁去修**：
后者是平台在上游欠费，用户只能等；前者只有用户自己能改，所以
`user_message` 必须直接把他指到设置页去，而不是"请稍后重试"。
上游到底是哪一类失败（鉴权/欠费/限流/参数）保留在 `detail.upstream_code`，
沿用"测试连接"那套分类，不另造一份。
...
**但共享的边界是账号，不是 Provider**（ADR-027）。熔断计的是"这个账号在
这家上游还能不能用"，所以作用域 id 是平台档一个、每个自带 Key 的 org
各一个（`cb:provider:<provider_id>[@org:<org_id>]`）。合成一把两个方向都错：
某个用户的 Key 被封会把整家 Provider 熔断掉、拖累所有用平台 Key 的人；
反过来平台账号出问题时也没道理拦住用自己 Key 的用户——那是另一个账号。
...
key_source                               -- platform / org，只记来源枚举
